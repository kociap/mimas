...
Mimas_Window* mimas_platform_create_window(Mimas_Window_Create_Info info)
{
  ...
  Mimas_X11_Window* const native_window =
    (Mimas_X11_Window*)malloc(sizeof(Mimas_X11_Window));
  memset(window, 0, sizeof(Mimas_Window));
  ...
  Screen* screen =
    mimas_XScreenOfDisplay(native_window->display, default_screen);

  native_window->window = mimas_XCreateSimpleWindow(
    native_window->display, screen->root, 0, 0, info.width, info.height, 1,
