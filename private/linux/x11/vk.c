...
typedef VkResult (*PFN_vkCreateXlibSurfaceKHR)(
  VkInstance, VkXlibSurfaceCreateInfoKHR const*,
...
mimas_bool mimas_platform_init_vk_backend(void)
{
  ...
  if(vulkan_module) {
    vkGetInstanceProcAddr =
      (PFN_vkGetInstanceProcAddr)dlsym(vulkan_module, "vkGetInstanceProcAddr");
    return mimas_true;
  } else {
    // TODO: Error
    return mimas_false;
...
{
  static char const* vk_win_extensions[] = {"VK_KHR_surface",
                                            "VK_KHR_xlib_surface", NULL};
  *extension_count = 2;
  ...
  VkSurfaceKHR* surface)
{
  ...
  Mimas_X11_Window* const native_window =
    (Mimas_X11_Window*)window->native_window;
  ...
  return vkCreateXlibSurfaceKHR(instance, &vk_win32_info, allocation_callbacks,
                                surface);
}
