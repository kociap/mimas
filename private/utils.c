...
Mimas_Rect _mimas_crop_rect(Mimas_Rect const* const source,
                            Mimas_Rect const* const target)
{
  Mimas_Rect cropped = {
    .left =
      _mimas_min_i32(_mimas_max_i32(target->left, source->left), target->right),
    .right = _mimas_min_i32(_mimas_max_i32(target->left, source->right),
                            target->right),
    .top =
      _mimas_min_i32(_mimas_max_i32(target->top, source->top), target->bottom),
    .bottom = _mimas_min_i32(_mimas_max_i32(target->top, source->bottom),
                             target->bottom)};
  return cropped;
}
...
static Codepoint_Conversion_Result
utf8_bytes_to_codepoint(mimas_char8 const* bytes)
{
...
mimas_char16* _mimas_utf8_to_utf16(mimas_char8 const* const buffer_utf8,
                                   mimas_i64 const count)
{
  mimas_i64 bytes_needed = 0;
  {
    ...
    while(mimas_true) {
      Codepoint_Conversion_Result const conv =
        utf8_bytes_to_codepoint(buffer_utf8_iter);
      ...
      mimas_bool const is_surrogate_pair =
        !(conv.codepoint <= 0xD7FF ||
          (conv.codepoint >= 0xE000 && conv.codepoint <= 0xFFFF));
      bytes_needed += 2 * (is_surrogate_pair + 1);
      if((count == -1 && conv.codepoint == U'\0') ||
  ...
  for(mimas_i64 bytes_read = 0; mimas_true;) {
    ...
    if(conv.codepoint <= 0xD7FF ||
       (conv.codepoint >= 0xE000 && conv.codepoint <= 0xFFFF)) {
      *buffer_utf16_iter = conv.codepoint;
      buffer_utf16_iter += 1;
    } else {
      mimas_char16 const high_surrogate =
        0xD800 + ((conv.codepoint - 0x10000) >> 10 & 0x3FF);
      mimas_char16 const low_surrogate =
        0xDC00 + ((conv.codepoint - 0x10000) & 0x3FF);
