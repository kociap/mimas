...
static BOOL enum_monitor_callback(HMONITOR hmonitor, HDC hdc, LPRECT rect,
                                  LPARAM lparam)
{
  UNUSED(hdc);
  UNUSED(rect);
...
Mimas_Display** _mimas_get_connected_displays(mimas_i64* const count)
{
  ...
  Mimas_Display** displays =
    (Mimas_Display**)malloc(display_array_size * sizeof(Mimas_Display*));
  if(!displays) {
    *count = 0;
    return NULL;
  }
  ...
  for(mimas_u32 adapter_index = 0;
      EnumDisplayDevicesW(NULL, adapter_index, &adapter_device, 0);
      adapter_index += 1) {
    DISPLAY_DEVICE display_device = {.cb = sizeof(DISPLAY_DEVICE)};
    for(mimas_u32 display_index = 0; EnumDisplayDevicesW(
          adapter_device.DeviceName, display_index, &display_device, 0);
        display_index += 1) {
      ...
      if(display_count == display_array_size) {
        Mimas_Display** r =
          realloc(displays, 2 * display_array_size * sizeof(Mimas_Display*));
        if(!r) {
      ...
      Mimas_Display* const display =
        (Mimas_Display*)malloc(sizeof(Mimas_Display));
      Mimas_Win_Display* const native_display =
        (Mimas_Win_Display*)malloc(sizeof(Mimas_Win_Display));
      ...
      WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS,
                          display_device.DeviceName, -1,
                          (char*)native_display->display_name,
                          ARRAY_SIZE(native_display->display_name), NULL, NULL);
      WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS,
                          adapter_device.DeviceName, -1,
                          (char*)native_display->adapter_name,
                          ARRAY_SIZE(native_display->adapter_name), NULL, NULL);
      ...
      EnumDisplayMonitors(NULL, NULL, enum_monitor_callback,
                          (LPARAM)&enum_monitor_data);
...
Mimas_Display_Settings mimas_get_display_settings(Mimas_Display* display)
{
  ...
  MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS,
                      (char*)native_display->adapter_name, -1, device_name, 32);
  DEVMODEW devmode = {0};
  EnumDisplaySettingsW(device_name, ENUM_CURRENT_SETTINGS, &devmode);
  Mimas_Display_Settings settings = {.width = devmode.dmPelsWidth,
