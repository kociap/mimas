...
static IShellItem* create_shell_item_from_path(Mimas_String_View path)
{
  ...
  SHCreateItemFromParsingName(wpath, NULL, &IID_IShellItem,
                              (void**)&shell_item);
...
Mimas_String mimas_open_file_dialog(Mimas_File_Dialog_Type type,
                                    Mimas_File_Dialog_Flags user_flags,
                                    Mimas_File_Filter* filters,
                                    mimas_u64 filter_count,
                                    Mimas_String_View const default_path)
{
  ...
  HRESULT hr = CoCreateInstance(cls_id, NULL, CLSCTX_INPROC_SERVER,
  ...
  if(user_flags & MIMAS_FILE_DIALOG_PICK_FILES) {
    flags |= FOS_FORCEFILESYSTEM;
  }
  if(user_flags & MIMAS_FILE_DIALOG_PICK_FOLDERS) {
  ...
  mimas_i64 const mb_buffer_size = WideCharToMultiByte(
    CP_UTF8, WC_ERR_INVALID_CHARS, result_path, -1, NULL, 0, NULL, NULL);
  mimas_char8* const buffer = malloc(mb_buffer_size);
  WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, result_path, -1,
                      (char*)buffer, mb_buffer_size, NULL, NULL);
  ...
  Mimas_String const string = {.begin = buffer,
                               .end = buffer + mb_buffer_size - 1};
