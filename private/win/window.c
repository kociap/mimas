...
{
  return WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_POPUP | WS_THICKFRAME |
...
static void clip_cursor_to_window(Mimas_Window* const window,
                                  Mimas_Rect const* const region)
{
  Mimas_Win_Window* const native_window =
    (Mimas_Win_Window*)window->native_window;
  RECT clip_region = {.left = region->left,
                      .top = region->top,
                      .right = region->right,
...
static void fit_window_to_display(Mimas_Window* const window,
                                  Mimas_Display* const display)
{
  Mimas_Win_Display* const native_display =
    (Mimas_Win_Display*)display->native_display;
  MONITORINFO info = {.cbSize = sizeof(MONITORINFO)};
  GetMonitorInfoW(native_display->hmonitor, &info);
  Mimas_Win_Window* const native_window =
    (Mimas_Win_Window*)window->native_window;
  SetWindowPos(native_window->handle, NULL, info.rcMonitor.left,
               info.rcMonitor.top, info.rcMonitor.right - info.rcMonitor.left,
               info.rcMonitor.bottom - info.rcMonitor.top,
               SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOCOPYBITS);
}

static mimas_i32 window_hit_test(mimas_i32 const cursor_x,
                                 mimas_i32 const cursor_y,
                                 RECT const window_rect)
{
  enum Region_Mask { client = 0, left = 1, right = 2, top = 4, bottom = 8 };
...
static Mimas_Key translate_scan_code(mimas_i32 const sc, mimas_bool const e0,
                                     mimas_bool const e1)
{
  UNUSED(e1);
  ...
  case 0x1d:
    return (MIMAS_KEY_RIGHT_CONTROL & (0 - e0)) |
           (MIMAS_KEY_LEFT_CONTROL & (0 - !e0));
  case 0x38:
  ...
  case 0x53:
    return (MIMAS_KEY_DELETE & (0 - e0)) |
           (MIMAS_KEY_NUMPAD_DECIMAL & (0 - !e0));
  case 0x35:
                           ...
                           LPARAM const lparam)
{
  ...
  if(global_input_handler) {
      ...
      case RIM_TYPEKEYBOARD: {
        ...
        if(key != MIMAS_KEY_UNKNOWN) {
          ...
          if(active_window && active_window->callbacks.key) {
            ...
            active_window->callbacks.key(active_window, key, action,
                                         active_window->callbacks.key_data);
          }
        }
      } break;
      ...
      case RIM_TYPEMOUSE: {
        ...
        if(mouse->usButtonFlags & RI_MOUSE_WHEEL) {
          if(_mimas->active_window && _mimas->active_window->callbacks.scroll) {
            ...
            _mimas->active_window->callbacks.scroll(
              _mimas->active_window, 0, delta,
              _mimas->active_window->callbacks.scroll_data);
          }
        }
        ...
        if(mouse_move_relative && _mimas->locked_cursor_window) {
          ...
          if(window->callbacks.cursor_pos) {
            window->callbacks.cursor_pos(window, _mimas->cursor_virtual_pos_x,
                                         _mimas->cursor_virtual_pos_y,
  ...
  } else {
    {
      ...
      mimas_bool const mouse_button_down =
        msg == WM_LBUTTONDOWN || msg == WM_RBUTTONDOWN ||
        msg == WM_MBUTTONDOWN || msg == WM_XBUTTONDOWN;
      // We clip the cursor on each mouse button event. The reason is that we might not clip it when a window is being activated due to
      // non-client activate (we want to avoid cursor clipping when the user is e.g. clicking the titlebar, dragging the window).
      if(window && _mimas->clipped_cursor_window == window &&
         mouse_button_down) {
        Mimas_Rect const client = get_client_rect(window);
        Mimas_Rect const clip_rect =
          _mimas_crop_rect(&_mimas->cursor_clip_region, &client);
    ...
    case WM_ACTIVATE: {
      ...
      if(active) {
        ...
        if(!native_window->non_client_activate) {
          if(_mimas->clipped_cursor_window == window) {
            Mimas_Rect const client = get_client_rect(window);
            Mimas_Rect const clip_rect =
              _mimas_crop_rect(&_mimas->cursor_clip_region, &client);
            clip_cursor_to_window(window, &clip_rect);
        ...
        if(window->callbacks.window_activate) {
          window->callbacks.window_activate(
            window, mimas_true, window->callbacks.window_activate_data);
        }
      } else {
        for(mimas_u32 i = 0; i < ARRAY_SIZE(window->keys); ++i) {
          if(window->keys[i] != MIMAS_KEY_RELEASE && window->callbacks.key) {
            window->callbacks.key(window, i, MIMAS_KEY_RELEASE,
        ...
        if(window->callbacks.window_activate) {
          window->callbacks.window_activate(
            window, mimas_false, window->callbacks.window_activate_data);
        }
    ...
    case WM_NCHITTEST: {
      ...
      if(window->callbacks.hittest) {
        Mimas_Rect const _window_rect = {window_rect.left, window_rect.top,
                                         window_rect.bottom, window_rect.right};
        Mimas_Rect const _client_rect = {client_rect.left, client_rect.top,
                                         client_rect.bottom, client_rect.right};
        Mimas_Hittest_Result const r = window->callbacks.hittest(
          window, GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam), _window_rect,
          _client_rect);
        LRESULT const hit[] = {
        ...
        return window_hit_test(GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam),
                               window_rect);
      }
    } break;
    ...
    case WM_SIZE: {
      ...
      if(window && window->callbacks.window_resize) {
        window->callbacks.window_resize(window, GET_X_LPARAM(lparam),
                                        GET_Y_LPARAM(lparam),
    ...
    case WM_LBUTTONUP: {
      if(window->callbacks.mouse_button) {
        window->callbacks.mouse_button(window, MIMAS_MOUSE_LEFT_BUTTON,
                                       msg == WM_LBUTTONDOWN,
                                       window->callbacks.mouse_button_data);
      }
      return TRUE;
    } break;
    ...
    case WM_RBUTTONUP: {
      if(window->callbacks.mouse_button) {
        window->callbacks.mouse_button(window, MIMAS_MOUSE_RIGHT_BUTTON,
                                       msg == WM_RBUTTONDOWN,
                                       window->callbacks.mouse_button_data);
      }
      return TRUE;
    ...
    case WM_MBUTTONUP: {
      if(window->callbacks.mouse_button) {
        window->callbacks.mouse_button(window, MIMAS_MOUSE_MIDDLE_BUTTON,
                                       msg == WM_MBUTTONDOWN,
                                       window->callbacks.mouse_button_data);
      }
      return TRUE;
    ...
    case WM_XBUTTONUP: {
      if(window->callbacks.mouse_button) {
        ...
        Mimas_Key const key =
          (MIMAS_MOUSE_THUMB_BUTTON_1 & (0 - !!(hw & XBUTTON1))) |
          (MIMAS_MOUSE_THUMB_BUTTON_2 & (0 - !!(hw & XBUTTON2)));
        window->callbacks.mouse_button(window, key, msg == WM_XBUTTONDOWN,
                                       window->callbacks.mouse_button_data);
      }
      return TRUE;
    } break;
    ...
    case WM_MOUSEMOVE: {
      if(!_mimas->locked_cursor_window && window->callbacks.cursor_pos) {
        window->callbacks.cursor_pos(window, GET_X_LPARAM(lparam),
                                     GET_Y_LPARAM(lparam),
                                     window->callbacks.cursor_pos_data);
      }
    ...
    case WM_CHAR: {
      Mimas_Win_Window* native_window =
        (Mimas_Win_Window*)window->native_window;
      ...
      if(native_window->cooking_character) {
        ...
        mimas_char32 const cooked_char =
          (((high_surrogate & 0x3FF) << 10) | (surrogate & 0x3FF)) - 0x35FDC00;
      ...
      if(window->callbacks.character) {
        ...
        native_window->cooking_character = mimas_false;
        window->callbacks.character(window, cooked_char,
                                    window->callbacks.character_data);
      }
...
static Mimas_Window* create_native_window(Mimas_Window_Create_Info const info)
{
  ...
  mimas_char16* wtitle = _mimas_utf8_to_utf16(info.title, -1);
  if(!wtitle) {
    free(window);
    return NULL;
  }
  mimas_u32 const style = WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_POPUP |
                          WS_THICKFRAME | WS_CAPTION | WS_SYSMENU |
                          WS_MAXIMIZEBOX | WS_MINIMIZEBOX;
  HWND const hwnd = CreateWindowEx(
    WS_EX_APPWINDOW, MIMAS_WINDOW_CLASS_NAME, wtitle, style, info.initial_pos_x,
    info.initial_pos_y, info.width, info.height, NULL, NULL, NULL, NULL);
  free(wtitle);
  ...
  Mimas_Win_Window* native_window =
    (Mimas_Win_Window*)malloc(sizeof(Mimas_Win_Window));
...
mimas_bool mimas_platform_init(Mimas_Backend const backend,
                               Mimas_Init_Options const* const options)
{
  ...
  Mimas_Win_Platform* const platform =
    (Mimas_Win_Platform*)malloc(sizeof(Mimas_Win_Platform));
  if(!platform) {
    // TODO: Error
    return mimas_false;
  }
  ...
  platform->enable_raw_input_sink =
    options->capture_input_when_application_is_out_of_focus;
  ...
  Mimas_Window_Create_Info dummy_window_create_info = {
    .title = (mimas_char8 const*)"MIMAS_HELPER_WINDOW",
    .width = 1280,
    .height = 720,
    .decorated = mimas_false};
  Mimas_Window* const dummy_window =
    create_native_window(dummy_window_create_info);
  if(!dummy_window) {
  ...
  HWND const dummy_hwnd =
    ((Mimas_Win_Window*)dummy_window->native_window)->handle;
  ...
  _mimas->default_cursor = mimas_create_standard_cursor(MIMAS_CURSOR_ARROW);
...
void mimas_platform_terminate(Mimas_Backend const backend)
{
  ...
  Mimas_Internal* const _mimas = _mimas_get_mimas_internal();
  mimas_destroy_cursor(_mimas->default_cursor);
    ...
    if(msg.message == WM_QUIT) {
      ...
      while(list_elem) {
        ...
        if(window->callbacks.close_requested) {
          window->callbacks.close_requested(
            window, window->callbacks.close_requested_data);
        }
...
void mimas_fullscreen_window(Mimas_Window* const window,
                             Mimas_Display* const display)
{
  ...
  if(display) {
    ...
    Mimas_Win_Window* const native_window =
      (Mimas_Win_Window*)window->native_window;
    SetWindowLongPtrW(native_window->handle, GWL_STYLE,
                      get_fullscreen_borderless_window_styles());
    fit_window_to_display(window, display);
    SetWindowPos(native_window->handle, HWND_TOP, 0, 0, 0, 0,
    ...
    Mimas_Win_Window* const native_window =
      (Mimas_Win_Window*)window->native_window;
    SetWindowLongPtrW(native_window->handle, GWL_STYLE,
                      get_default_window_styles());
    SetWindowPos(native_window->handle, NULL, 0, 0, 0, 0,
...
void mimas_set_window_title(Mimas_Window* const window,
                            mimas_char8 const* title)
{
  Mimas_Win_Window* const native_window =
    (Mimas_Win_Window*)window->native_window;
  mimas_char16* const wtitle = _mimas_utf8_to_utf16(title, -1);
  if(wtitle) {
...
void mimas_set_window_pos(Mimas_Window* const window, mimas_i32 const x,
                          mimas_i32 const y)
{
  if(!window->display) {
    Mimas_Win_Window* const native_window =
      (Mimas_Win_Window*)window->native_window;
    SetWindowPos(native_window->handle, NULL, x, y, 0, 0,
...
void mimas_get_window_pos(Mimas_Window* const window, mimas_i32* const x,
                          mimas_i32* const y)
{
  Mimas_Win_Window* const native_window =
    (Mimas_Win_Window*)window->native_window;
...
void mimas_set_window_content_pos(Mimas_Window* const window, mimas_i32 const x,
                                  mimas_i32 const y)
{
  if(!window->display) {
    Mimas_Win_Window* const native_window =
    ...
    if(window->decorated) {
      RECT pos = {x, y, x, y};
      // TODO: Make DPI aware.
      AdjustWindowRectEx(&pos, get_default_window_styles(), FALSE,
                         get_default_extended_window_styles());
      SetWindowPos(native_window->handle, NULL, pos.left, pos.top, 0, 0,
                   SWP_NOSIZE | SWP_NOOWNERZORDER | SWP_NOZORDER |
                     SWP_NOACTIVATE);
    } else {
      SetWindowPos(native_window->handle, NULL, x, y, 0, 0,
...
void mimas_get_window_content_pos(Mimas_Window* const window,
                                  mimas_i32* const x, mimas_i32* const y)
{
  Mimas_Win_Window* const native_window =
    (Mimas_Win_Window*)window->native_window;
  POINT pos = {0, 0};
...
void mimas_set_window_content_size(Mimas_Window* const window,
                                   mimas_i32 const width,
                                   mimas_i32 const height)
{
  if(!window->display) {
    Mimas_Win_Window* const native_window =
      (Mimas_Win_Window*)window->native_window;
    if(window->decorated) {
      ...
      // TODO: Make DPI aware.
      AdjustWindowRectEx(&rect, get_default_window_styles(), FALSE,
                         get_default_extended_window_styles());
      SetWindowPos(native_window->handle, NULL, 0, 0, rect.right - rect.left,
                   rect.bottom - rect.top,
                   SWP_NOMOVE | SWP_NOOWNERZORDER | SWP_NOZORDER |
                     SWP_NOACTIVATE);
    } else {
      SetWindowPos(native_window->handle, NULL, 0, 0, width, height,
                   SWP_NOMOVE | SWP_NOOWNERZORDER | SWP_NOZORDER |
                     SWP_NOACTIVATE);
    }
...
void mimas_get_window_content_size(Mimas_Window* const window,
                                   mimas_i32* const width,
                                   mimas_i32* const height)
{
  Mimas_Win_Window* const native_window =
...
void mimas_clip_cursor(Mimas_Window* const window,
                       Mimas_Rect const* const region)
{
  Mimas_Internal* const _mimas = _mimas_get_mimas_internal();
  if(_mimas->locked_cursor_window) {
    return;
  }

  if(region) {
    Mimas_Rect corrected_region = *region;
    corrected_region.left = _mimas_min_i32(region->left, region->right);
    corrected_region.top = _mimas_min_i32(region->top, region->bottom);
    _mimas->cursor_clip_region = corrected_region;
    if(_mimas->active_window == window) {
      // Correct the region so that it doesn't extend outside the client area
      Mimas_Rect const client = get_client_rect(window);
      Mimas_Rect const clip_region =
        _mimas_crop_rect(&corrected_region, (Mimas_Rect*)&client);
...
{
  Mimas_Internal* const _mimas = _mimas_get_mimas_internal();
  if(_mimas->locked_cursor_window || _mimas->clipped_cursor_window) {
    return;
  }
